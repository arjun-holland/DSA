...
Difficulty: MediumAccuracy: 60.4%Submissions: 23K+Points: 4
...
public:
    vector<int> findGreater(vector<int>& arr) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            while (!st.empty() && freq[arr[st.top()]] <= freq[arr[i]]) {    // Pop elements from stack while frequency at stack top is <= current
                st.pop();
            }

            if (!st.empty()) {      // If stack is not empty, top is the next greater frequency element
                result[i] = arr[st.top()];
            }
